...
");
    ...
    wmm_lines = (char**)malloc(sizeof(char*) * 256);
    wmm_index = 0;
...
double dayTime(int year, int momth, int day)
{
    ...
    sum += day;
    if ((year % 4 == 0 && year % 100 != 0 || year % 400 == 0) && momth > 2)
    {
        sum++;
    }
    return year + (1.0 * sum / 365.0); // day 94 of year 2020
}


double decCalculator(double alt, double glat, double glon, int year, int momth, int day)
{
    geomag_init();
    double time = dayTime(year, momth, day);
