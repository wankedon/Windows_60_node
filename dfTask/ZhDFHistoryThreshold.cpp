...
DFHistoryThreshold::DFHistoryThreshold(const spectrum::FrequencyBand& wholeBand, std::shared_ptr<persistence::DFDBAccessor> accessor)
...
DFHistoryThreshold::~DFHistoryThreshold() = default;

bool DFHistoryThreshold::loadHistory(const detection::HistoryThresholdParam& param)
{
	...
	if (taskIds.empty())
		return false;
	persistence::DFDataItemHandler handler = [this](const persistence::DFDataItem& item) {thresholdLine += item.spectrum; };
